...
import { signOut, changeContacts } from '../store/auth/actions';
import {
...
import SearchContact from '../components/Sidebar/InputSearchContact';
...
const Messenger = props => {
  ...
  const prevMessages = appStore.messages
    ? JSON.stringify(appStore.messages)
    : null;

  const observer = firebase
    .database()
    .ref(`users/${userName}/messagesList/${recipient}`);
  ...
  const handleAddContact = contactName => {
    const setContacts = new Set(contacts);
    setContacts.add(contactName);
    changeContacts([...setContacts], userName).then(() => {
      message.info('Контакт добавлен');
    });
  };

  const handleRemoveContact = contactName => {
    const filterContacts = contacts.filter(contact => contact !== contactName);
    changeContacts([...filterContacts], userName).then(() => {
      message.info('Контакт удален');
    });
  };
...
const mapStateFromProps = ({ authStore, appStore }) => ({
  authStore,
  appStore,
});
